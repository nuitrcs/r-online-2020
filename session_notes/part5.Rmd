...
```{r}
library(dplyr)  # we'll use the bind_rows function
```
...
```{r}
my_plot <- function(data, xlab) {
    hist(data, 
         col = "pink",
         breaks = 20,
         border = FALSE,  # no line around bars
         main = "Cars Dataset", 
         xlab = xlab)  # get the value from input
}
...
list.files("data/pdb",   # where the files are
           "*.pdb",   # the pattern to match filenames against (optional)
